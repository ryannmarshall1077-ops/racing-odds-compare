...
- [x] Upcoming Races now loads every AU race today, not just the next 20
      ...
      - **Follow-up, same session**: user-reported a specific race (Wagga
        greyhound R1) showing the "!" no-Sportsbet-match warning despite
        being genuinely live and priced on Sportsbet's own site right
        then. Root-caused by directly querying Sportsbet's real
        `NextEvents` API (the JSON feed `fetchSportsbetNextEvents` used):
        it hard-caps *each* racing category (HR/HA/GH, domestic and
        international, six categories total) at exactly 40 events,
        sorted soonest-first — confirmed by requesting `GH_DOMESTIC`
        alone and getting back exactly 40, cut off at a time well before
        Wagga's own race. Tried the obvious pagination params
        (`count`/`limit`/`take`/`pageSize`) against the live endpoint —
        none raised it. Once this extension started loading every race
        today instead of just the next 20, this cap started mattering:
        any race further out than the nearest ~40-per-sport window got a
        false "no match" warning even though it was genuinely on
        Sportsbet, and would keep getting it until the 60s poll's window
        finally slid forward enough to include it.

        Found a full replacement while digging further: Sportsbet's own
        `/racing-schedule` page embeds its *entire* day's schedule,
        uncapped (verified: 682 events across 67 competitions in the
        same real check, vs `NextEvents`' 120-per-request ceiling) — not
        as a separate REST endpoint, but inline in the page's own
        server-rendered HTML as `window.__PRELOADED_STATE__`. Rewrote
        `fetchSportsbetNextEvents` (renamed
        `fetchSportsbetRaceSchedule`, matching what it actually fetches
        now) to fetch that page directly and pull the object out with a
        brace-depth-aware parser (`extractPreloadedState` — tracks
        string/escape state so it can't be fooled by a literal "}" or
        "</script>"-looking text inside a JSON string value, unlike a
        fixed-size slice or a naive regex) rather than calling
        `NextEvents` at all. Returns the exact same event shape
        (`{id, type, competitionName, raceNumber, startTime}`) the
        existing matching logic and `buildSportsbetRaceUrl` already
        expected, so neither needed to change — classId->sport/region
        fields (`SPORTSBET_CLASS_ID_TYPE`) needed for the mapping were
        themselves verified directly (classId 1/3/4 -> AU horse/harness/
        greyhound domestic tracks; 2/112/134/355 -> each sport's
        INTERNATIONAL counterpart, correctly excluded). Heavier per call
        (~1.7MB of HTML vs a small JSON payload) but still one request at
        the same ~1/minute cadence as before.

        Verified end-to-end against the live site: ran the exact new
        `fetchSportsbetRaceSchedule`/matching/`buildSportsbetRaceUrl`
        code together, got a real match for Wagga R1 that `NextEvents`
        couldn't provide, and confirmed the constructed URL actually
        loads that exact race (page title matched).
